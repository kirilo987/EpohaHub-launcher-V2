document.addEventListener('DOMContentLoaded', function() {
	window.addEventListener('pywebviewready', function() {
		...
		pywebview.api.get_last_account().then(function(result) {
			...
			if (result == "") {
				lastuser.textContent = "Ви ввійшли як: Не ввійшли!";
				localStorage.setItem('current-username', '');
			} else {
				lastuser.textContent = "Увійшли як: " + result;
		...
		setTimeout(function() {
			pywebview.api.get_recentinstance().then(function(result) {
				if (result == "") {
					lastinst.textContent = "Поточна версія: не вибрано";
				} else {
...
window.addEventListener('message', function(event) {
    ...
    if (event.data == 'accountchange') {
    	var lastuser = document.getElementById('lastuser');
    	lastuser.textContent = "Увійшли як: " + localStorage.getItem('current-username');
